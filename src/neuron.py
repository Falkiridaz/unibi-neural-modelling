...
class Neuron:
    ...
    def create_morphology(self, num_dendrites: int = 3, dendrite_lengths: list[float] = None) -> tuple[int, int, list[int]]:
        ...
        if dendrite_lengths is None: # default lengths
            ...
            dendrite_lengths.extend([last_length] * (num_dendrites - len(dendrite_lengths)))
        ...
        for i in range(num_dendrites):
            ...
            dendrite_id = self.add_compartment(CompartmentType.DENDRITE, dendrite_params)
    ...
    def calculate_axon_currents(self):
        ...
        for comp_id, compartment in enumerate(self.compartments):
            for neighbor_id in self.connections[comp_id]:
                ...
                R_axial = (compartment.params.R_a * compartment.params.length + neighbor.params.R_a * neighbor.params.length) / 2 # axial resistance
                I_axial = (neighbor.state.V - compartment.state.V) / R_axial # Ohms law
    ...
    def add_synapse(self, compartment_id: int, tau_rise: float = 0.5, tau_decay: float = 5.0, amplitude: float = 1.0, reversal_potential: float = 0.0) -> Synapse:
        ...
        synapse = Synapse(tau_rise, tau_decay, amplitude, reversal_potential, self.params.dt)
    ...
    def add_spike_to_synapse(self, compartment_id: int, synapse_index: int, spike_time: float):
        ...
        if (0 <= compartment_id < len(self.compartments) and 0 <= synapse_index < len(self.compartments[compartment_id].synapses)):
            self.compartments[compartment_id].synapses[synapse_index].add_spike(spike_time)
    

    def get_voltage_histories(self) -> dict[int, np.ndarray]: return {i: comp.get_voltage_history() for i, comp in enumerate(self.compartments)}
    
    def get_spike_times(self) -> dict[int, list[float]]: return {i: comp.get_spike_times() for i, comp in enumerate(self.compartments)}
    
    def get_soma_voltage(self) -> float: return 0.0 if self.soma_id is None else self.compartments[self.soma_id].state.V
    
    def get_axon_voltage(self) -> float: return 0.0 if self.axon_id is None else self.compartments[self.axon_id].state.V
    
    def has_soma_spiked(self) -> bool: return False if self.soma_id is None else self.compartments[self.soma_id].has_spiked()
    
    def has_axon_spiked(self) -> bool: return False if self.axon_id is None else self.compartments[self.axon_id].has_spiked()

...
LAYER_NAMES = ["Input", "Input", "Hidden1", "Hidden1", "Hidden1", "Hidden2", "Hidden2", "Output"]
...
def run_demo():
    ...
    network.add_external_input(neuron_id=0, compartment_id=soma_id_0, t_start=5.0, t_end=15.0, amplitude=0.05)
    network.add_external_input(neuron_id=1, compartment_id=soma_id_1, t_start=50.0, t_end=60.0, amplitude=0.01)
...
def create_plots(network, layers):    
    ...
    plt.suptitle('Multi-Compartment Neuronal Network Model', fontsize=16, fontweight='bold', y=0.98)
    plt.savefig('../imgs/neural_network_demo.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_network_architecture(ax, network, layers):
    ...
    node_colors = []
    for i in range(len(network.neurons)):
        if   i in layers[0]: node_colors.append('lightblue')
        elif i in layers[1]: node_colors.append('lightgreen')
        elif i in layers[2]: node_colors.append('lightcoral')
        elif i in layers[3]: node_colors.append('gold')
        else:                node_colors.append('lightgray')
    
    nx.draw(G, pos, ax=ax, with_labels=True, node_color=node_colors, node_size=900, arrows=True, arrowstyle='->', arrowsize=20, edge_color='gray', width=2, alpha=0.7)
...
def plot_firing_rates(ax, network):
    ...
    for neuron in network.neurons:
        soma_id = neuron.soma_id
        spike_times = neuron.compartments[soma_id].get_spike_times()
        firing_rate = len(spike_times) / (network.t / 1000.0) if network.t > 0 else 0
        firing_rates.append(firing_rate)
    
    bars = ax.bar(range(8), firing_rates, color=COLORS, alpha=0.7, edgecolor='black')
    ...
    for i, (bar, rate) in enumerate(zip(bars, firing_rates)):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.1, f'{rate:.1f}', ha='center', va='bottom', fontsize=8, fontweight='bold')
...
def plot_parameter_comparison(ax, network):
    ...
    refractory_periods = [neuron.params.refractory_period for neuron in network.neurons]
    
    ax.scatter(thresholds, refractory_periods, c=range(8), cmap='plasma', s=100, alpha=0.7)
    ...
    for (thresh, ref), neuron_indices in param_groups.items():
        if len(neuron_indices) == 1:
            ax.annotate(f'N{neuron_indices[0]}', (thresh, ref), xytext=(5, 5), textcoords='offset points', fontsize=8)
        else:
            for j, neuron_idx in enumerate(neuron_indices):
                ...
                ax.annotate(f'N{neuron_idx}', (thresh, ref), xytext=(offset_x, offset_y), textcoords='offset points', fontsize=8, ha='center', va='center')
...
def plot_network_statistics(ax, network):
    ...
    ax.text(0.05, 0.95, text, transform=ax.transAxes, fontsize=10, verticalalignment='top', fontfamily='monospace', bbox=dict(boxstyle='round', facecolor='lightgray', alpha=0.8))
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis('off')
...
def print_summary(network):
    ...
    print(f"- Network activity: {sum(1 for rate in firing_rates if rate > 0)} active neurons")

...
class Compartment:    
    ...
    def _beta_n(self, V: float) -> float: return 0.125 * np.exp(np.clip(-0.0125 * (V + 65), -50, 50))
    ...
    def _beta_m(self, V: float) -> float: return 4.0 * np.exp(np.clip(-0.0556 * (V + 65), -50, 50))

    def _alpha_h(self, V: float) -> float: return 0.07 * np.exp(np.clip(-0.05 * (V + 65), -50, 50))

    def _beta_h(self, V: float) -> float: return 1 / (1 + np.exp(np.clip(-0.1 * (V + 35), -50, 50)))
    ...
    def _beta_Ca(self, V: float) -> float: return 0.1 * np.exp(np.clip(-0.05 * (V + 20), -50, 50))

    def _alpha_K_Ca(self, Ca: float) -> float: return 0.1 * Ca / (Ca + 0.1)
    def _beta_K_Ca(self, Ca: float) -> float: return 0.1

    def _alpha_h_channel(self, V: float) -> float: return 0.1 * np.exp(np.clip(0.05 * (V + 80), -50, 50))
    def _beta_h_channel(self, V: float) -> float: return 0.1 / (1 + np.exp(np.clip(0.05 * (V + 80), -50, 50)))
    ...
    def _beta_Kv(self, V: float) -> float: return 0.1 * np.exp(np.clip(-0.05 * (V + 30), -50, 50))
    ...
    def update(self, I_ext: float = 0.0) -> bool:
        ...
        I_syn = sum(synapse.get_current(self.state.V) for synapse in self.synapses)
        I_membrane = I_Na + I_K + I_L + I_Ca + I_K_Ca + I_h + I_Kv + I_syn
        I_axial = self.state.I_axial_in - self.state.I_axial_out
        
        dV_dt = (I_ext + I_axial - I_membrane) / (self.params.C_m * self.params.area * 1e-8)
    ...
    def _calculate_dendritic_currents(self) -> tuple[float, float, float, float]:
        ...
        if self.params.g_K_Ca > 0 and self.state.Ca > 0:
            ...
            if alpha_K_Ca + beta_K_Ca > 0:
                g_K_Ca = self.params.g_K_Ca * alpha_K_Ca / (alpha_K_Ca + beta_K_Ca)
    ...
    def _update_gating_variables(self):
        ...
        # Update m, h, n
        self.state.m += self.dt * (self._alpha_m(V) * (1 - self.state.m) - self._beta_m(V) * self.state.m)
        self.state.h += self.dt * (self._alpha_h(V) * (1 - self.state.h) - self._beta_h(V) * self.state.h)
        self.state.n += self.dt * (self._alpha_n(V) * (1 - self.state.n) - self._beta_n(V) * self.state.n)
    ...
    def _update_calcium_concentration(self, I_Ca: float):
        if self.params.g_Ca > 0 and self.params.area > 0:
            ...
            self.state.Ca = max(0, self.state.Ca + self.dt * (ca_influx - ca_decay))
    ...
    def _has_spiked(self) -> bool:
        ...
        spike_threshold = self.params.spike_threshold if hasattr(self.params, 'spike_threshold') else 0.0
        
        # threshold crossed from below AND not during refractory period
        spiked = (self.last_V < spike_threshold and self.state.V >= spike_threshold) and not in_refractory
    ...
    def get_voltage_history(self) -> np.ndarray: return np.array(self.V_history)
    
    def get_spike_times(self) -> list[float]: return self.spike_times.copy()
    
    def has_spiked(self) -> bool: return (len(self.spike_times) > 0 and self.spike_times[-1] == self.state.t - self.dt)

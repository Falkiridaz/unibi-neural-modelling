...
class NeuronalNetwork:
    ...
    def __init__(self, num_neurons: int = 5, neuron_params: Optional[NeuronParameters] = None, neuron_params_list: Optional[list[NeuronParameters]] = None):
        ...
        if neuron_params_list is not None:
            if len(neuron_params_list) != num_neurons:
                raise ValueError(f"neuron_params_list length ({len(neuron_params_list)}) must match num_neurons ({num_neurons})")
        ...
        for i in range(num_neurons):
            ...
            neuron.create_morphology(num_dendrites=3, dendrite_lengths=[200.0] * 3)
        ...
        self.t = 0.0
        self.dt = self.neuron_params_list[0].dt # use dt from first neuron for network
    ...
    def connect_neurons(self, pre_neuron_id: int, post_neuron_id: int, weight: float = 1.0, delay: float = 1.0, post_compartment_id: Optional[int] = None, synapse_params: Optional[SynapticParameters] = None) -> int:
        ...
        self.neurons[post_neuron_id].add_synapse(post_compartment_id, synapse_params.tau_rise, synapse_params.tau_decay, synapse_params.amplitude * weight, synapse_params.reversal_potential)
        synapse_index = len(self.neurons[post_neuron_id].compartments[post_compartment_id].synapses) - 1
        
        connection = Connection(pre_neuron_id=pre_neuron_id, post_neuron_id=post_neuron_id, post_compartment_id=post_compartment_id, synapse_index=synapse_index, weight=weight, delay=delay)
    ...
    def _propagate_spike(self, pre_neuron_id: int, spike_time: float):
        for connection in self.connections:
            if connection.pre_neuron_id == pre_neuron_id:
                ...
                self.neurons[connection.post_neuron_id].add_spike_to_synapse(connection.post_compartment_id, connection.synapse_index, delayed_spike_time)
    ...
    def get_network_statistics(self) -> dict:
        ...
        for i, neuron in enumerate(self.neurons):
            ...
            neuron_stat = {
                ...
                'soma_firing_rate': len(soma_spikes) / (self.t / 1000.0) if self.t > 0 else 0, # ms -> s
    ...
    def create_feedforward_network(self, layer_sizes: list[int], connection_weights: list[float] = None, clear_existing: bool = True) -> list[list[int]]:
        ...
        if sum(layer_sizes) != self.num_neurons:
            raise ValueError(f"Total layer size ({sum(layer_sizes)}) must equal num_neurons ({self.num_neurons})")
        
        if clear_existing: self.clear_network()
        ...
        if connection_weights is None:
            ...
            connection_weights.extend([last_weight] * (total_connections - len(connection_weights)))
        ...
        for i in range(len(layers) - 1):
            ...
            for pre_neuron in current_layer:
                for post_neuron in next_layer:
                    self.connect_neurons(pre_neuron, post_neuron, connection_weights[weight_idx])
                    weight_idx += 1
